import { fetchClienteDetail, fetchEquipe } from "@/lib/queries/admin";
...
export default async function ClienteDetailPage({ params }: { params: { id: string } }) {
  const [cliente, equipe] = await Promise.all([fetchClienteDetail(params.id), fetchEquipe()]);
  if (!cliente) notFound();
  ...
  return (
    <div className="max-w-4xl">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 mb-6 flex items-start gap-4">
        <div className="w-14 h-14 rounded-xl flex items-center justify-center text-white text-xl font-bold shrink-0" style={{ background: couleurPrincipale }}>
        ...
        <div className="flex flex-col gap-6">
            ...
            {forfaits.length === 0 ? (
              ...
              <div className="flex flex-col gap-3">
                {forfaits.map((f) => {
                  ...
                  return (
                    <div key={f.id} className={`rounded-xl border p-4 ${actif ? "border-slate-200 bg-slate-50" : "border-dashed border-slate-200 opacity-60"}`}>
                      <div className="flex justify-between items-start mb-2">
                        <p className="font-medium text-slate-700 text-sm leading-snug">{f.forfait_nom}</p>
                        <StatusChip restantes={f.seances_restantes} jours={jours} />
                      </div>
                      <div className="flex gap-1.5 flex-wrap mb-2">
                        {Array.from({ length: f.nombre_seances_total }).map((_, i) => (
                          <span key={i} className="w-3.5 h-3.5 rounded-full border-2" style={{ borderColor: couleurPrincipale, background: i < f.seances_restantes ? couleurPrincipale : "transparent", opacity: i < f.seances_restantes ? 1 : 0.25 }} />
                        ))}
                        <span className="text-xs font-semibold text-slate-600 ml-1">{f.seances_restantes}/{f.nombre_seances_total}</span>
                      </div>
                      <p className="text-xs text-slate-400">
                        Expire le {fmt(f.date_expiration)}
                        {actif && jours <= 30 && <span className="text-orange-600 font-medium"> (dans {jours} j)</span>}
                      </p>
                    </div>
                  );
                })}
          ...
          <Section title="Marquer une prestation">
            {forfaitsDisponibles.length === 0 ? (
              <p className="text-sm text-slate-400 py-2">Aucun forfait avec séances disponibles.</p>
            ) : (
              <MarquerPrestationForm
                clienteId={cliente.id}
                forfaits={forfaitsDisponibles.map((f) => ({ id: f.id, nom: f.forfait_nom, restantes: f.seances_restantes }))}
              ...
              {prestations.map((p) => (
                <div key={p.id} className="flex gap-3 items-start rounded-xl border border-slate-100 bg-slate-50 px-4 py-3">
                  <div className="shrink-0 mt-0.5">
                    <div className="w-2.5 h-2.5 rounded-full mt-1" style={{ background: couleurPrincipale }} />
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex justify-between gap-2 flex-wrap">
                      <p className="text-sm font-medium text-slate-700">{fmt(p.date_prestation)}</p>
                      {p.estheticienne && <span className="text-xs text-slate-400">{p.estheticienne}</span>}
                    </div>
                    {p.forfait_nom && <p className="text-xs mt-0.5" style={{ color: couleurPrincipale }}>{p.forfait_nom}</p>}
...
function StatusChip({ restantes, jours }: { restantes: number; jours: number }) {
  if (restantes === 0 || jours < 0) return <span className="text-[10px] bg-slate-100 text-slate-500 px-2 py-0.5 rounded-full font-medium">Terminé</span>;
  if (restantes <= 1) return <span className="text-[10px] bg-amber-100 text-amber-700 px-2 py-0.5 rounded-full font-semibold">1 séance</span>;
  if (jours <= 30) return <span className="text-[10px] bg-orange-100 text-orange-700 px-2 py-0.5 rounded-full font-semibold">⏳ {jours}j</span>;
