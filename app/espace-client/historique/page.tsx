...
import { fetchClienteByEmail, fetchHistoriqueCliente } from "@/lib/queries/cliente";
...
// Regroupe les prestations par année/mois
function groupByMonth(prestations: Awaited<ReturnType<typeof fetchHistoriqueCliente>>) {
  const groups: Record<string, typeof prestations> = {};
  for (const p of prestations) {
    const d = new Date(p.date_prestation);
...
export default async function HistoriquePage() {
  ...
  const prestations = await fetchHistoriqueCliente(cliente.id);
  const { couleurPrincipale } = config.branding;
  const groups = groupByMonth(prestations);
      ...
      {prestations.length === 0 ? (
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-10 text-center">
          ...
          <p className="text-sm text-gray-400 max-w-xs mx-auto">
            Votre historique de soins apparaîtra ici après votre premier rendez-vous.
          </p>
        </div>
      ) : (
          ...
          {Object.entries(groups).map(([mois, items]) => (
            <section key={mois}>
              ...
              <div className="flex items-center gap-3 mb-4">
                <h2 className="text-sm font-semibold text-gray-500 uppercase tracking-widest capitalize">
                  {mois}
                </h2>
                <div className="flex-1 h-px bg-gray-100" />
                <span
                  className="text-xs font-medium px-2.5 py-1 rounded-full"
                  style={{ color: couleurPrincipale, background: `${couleurPrincipale}15` }}
                >
              ...
              <div className="relative flex flex-col gap-3 pl-5">
                ...
                <div
                  ...
                  style={{ background: `${couleurPrincipale}30` }}
                  aria-hidden
                />
                ...
                {items.map((p) => (
                  <div key={p.id} className="relative">
                    {/* Point */}
                    <div
                      className="absolute -left-5 top-3.5 w-3 h-3 rounded-full border-2 bg-white"
                      style={{ borderColor: couleurPrincipale }}
                      aria-hidden
                    />
                    ...
                    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
                      <div className="flex items-start justify-between gap-3 flex-wrap">
                        <div>
                          <p className="font-semibold text-gray-800 text-sm capitalize">
                            {formatDate(p.date_prestation)}
                          </p>
                          {p.forfaits_clientes?.forfaits?.nom && (
                            <p
                      ...
                      {p.notes && (
                        <p className="mt-3 text-xs text-gray-500 bg-gray-50 rounded-xl px-3 py-2 leading-relaxed italic">
                          &ldquo;{p.notes}&rdquo;
                        </p>
                      )}
                    </div>
